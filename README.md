<div align="center">

# 🚀 Awesome NCNN Collection

<img src="https://img.shields.io/badge/NCNN-Framework-blue?style=for-the-badge&logo=tensorflow&logoColor=white" alt="NCNN">
<img src="https://img.shields.io/badge/Status-Active-success?style=for-the-badge" alt="Status">
<img src="https://img.shields.io/badge/Updated-2025-orange?style=for-the-badge" alt="Updated">
<img src="https://img.shields.io/badge/License-MIT-green?style=for-the-badge" alt="License">

### ⚡ High-Performance Neural Network Inference for Mobile & Edge AI

> *Your ultimate curated collection of cutting-edge NCNN resources, projects, and implementations*

[🌟 Star](../../) • [🔥 Trending Projects](#-latest-yolo-implementations-2024-2025) • [📱 Mobile Deploy](#-platform-specific-deployment) • [⚙️ Optimization](#-optimization--quantization)

</div>

---

## 📊 Performance Highlights

<div align="center">

| Metric | Performance | vs GPU |
|:------:|:-----------:|:------:|
| ⚡ Latency | **58.54% Lower** | Matrix Operations |
| 🚄 LLM Inference | **3.2× Faster** | Inference Speed |
| 💪 Throughput | **2× Higher** | vs NPUs |
| 📱 Platforms | **10+ Supported** | Cross-Platform |

</div>

---

## 📑 Table of Contents

<details open>
<summary><b>🗂️ Click to expand/collapse</b></summary>

- [🎯 Official NCNN Resources](#-official-ncnn-resources)
- [🔥 Latest YOLO Implementations (2024-2025)](#-latest-yolo-implementations-2024-2025)
- [🎯 Object Detection](#-object-detection)
- [🖼️ Image Classification](#️-image-classification)
- [✂️ Image Segmentation](#️-image-segmentation)
- [✨ Super Resolution & Image Enhancement](#-super-resolution--image-enhancement)
- [👤 Face & Biometrics](#-face--biometrics)
- [🏃 Pose Estimation & Human Tracking](#-pose-estimation--human-tracking)
- [🎙️ Speech Recognition & ASR](#️-speech-recognition--asr)
- [📝 OCR & Text Recognition](#-ocr--text-recognition)
- [🎬 Video Processing](#-video-processing)
- [🎨 Stable Diffusion & Generative Models](#-stable-diffusion--generative-models)
- [📦 Model Collections & Zoo](#-model-collections--zoo)
- [📱 Platform-Specific Deployment](#-platform-specific-deployment)
- [⚙️ Optimization & Quantization](#️-optimization--quantization)
- [🛠️ Tools & Utilities](#️-tools--utilities)
- [📚 Tutorials & Documentation](#-tutorials--documentation)
- [📊 Benchmarks & Performance](#-benchmarks--performance)
- [📄 Research Papers (2024-2025)](#-research-papers--articles-2024-2025)

</details>

---

## 🎯 Official NCNN Resources

### 🏆 Core Framework

<table>
<tr>
<td width="50%">

#### 🔧 Main Repository
- ⭐ [**Tencent/ncnn**](https://github.com/Tencent/ncnn)
  - ![Stars](https://img.shields.io/github/stars/Tencent/ncnn?style=social)
  - 22k+ stars - Official framework
  - Mobile-optimized inference

</td>
<td width="50%">

#### 📖 Documentation
- 📘 [**Official Docs**](https://ncnn.readthedocs.io/en/latest/)
- 📙 [**DocsForge**](https://ncnn.docsforge.com/)
- 🌋 [**Vulkan FAQ**](https://github.com/Tencent/ncnn/wiki/FAQ-ncnn-vulkan)
- 📊 [**Benchmarks**](https://github.com/Tencent/ncnn/tree/master/benchmark)

</td>
</tr>
</table>

### 🐍 Python Integration
- 🔗 [**NCNN Python Wrapper**](https://github.com/Tencent/ncnn/tree/master/python) - Official Python bindings

---

## 🔥 Latest YOLO Implementations (2024-2025)

<div align="center">

### 🏅 YOLOv11 (Latest - 2024)

![YOLOv11](https://img.shields.io/badge/YOLOv11-New-red?style=for-the-badge&logo=yolo)
![Performance](https://img.shields.io/badge/Inference-48ms-brightgreen?style=for-the-badge)
![Platform](https://img.shields.io/badge/Platform-Mobile-blue?style=for-the-badge)

</div>

#### 📱 Production-Ready Implementations

| Project | Platform | Performance | Description |
|---------|----------|-------------|-------------|
| 📘 [**Ultralytics Official**](https://docs.ultralytics.com/integrations/ncnn/) | All | ⚡ Optimized | Official NCNN export guide |
| ⚙️ [**yolo11-ncnn**](https://github.com/zhouweigogogo/yolo11-ncnn) | C++ | 48ms | High-performance C++ impl |
| 🎯 [**No Magic Ops**](https://github.com/taiji1985/yolo11-ncnn) | All | ✅ Clean | Direct Ultralytics export |
| 🤖 [**Android YOLOv11**](https://github.com/gaoxumustwin/ncnn-android-yolov11) | Android | 📱 Mobile | Optimized for Android |
| 🎥 [**Real-time Demo**](https://github.com/Abandon-ht/ncnn-android-yolo11) | Android | 🚀 Fast | Live detection demo |
| 🤖 [**ROS 2 Support**](https://github.com/mlim97/yolo_ros) | ROS | 🔧 Robotics | YOLOv8/v9/v10/v11 |

### 🎖️ YOLOv10 & YOLOv9 (2024)
- 📊 [**Performance Comparison**](https://www.ultralytics.com/blog/comparing-ultralytics-yolo11-vs-previous-yolo-models) - v8/v9/v10/v11 benchmarks

### 🥈 YOLOv8 & Earlier Versions

<details>
<summary>📦 <b>Click to expand YOLOv5-v8 implementations</b></summary>

- 🎯 [**YOLOv8 Android**](https://github.com/FeiGeChuanShu/ncnn-android-yolov8)
- 🎯 [**YOLOv6 Android**](https://github.com/FeiGeChuanShu/ncnn-android-yolov6)
- ✂️ [**YOLOv5 Segmentation**](https://github.com/FeiGeChuanShu/yolov5-seg-ncnn)
- 📱 [**YOLOv6 + Guide**](https://github.com/DataXujing/ncnn_android_yolov6)
- 🍎 [**YOLOv5 iOS/Android**](https://github.com/cmdbug/YOLOv5_NCNN)
- ⚡ [**YOLOv8 TensorRT**](https://github.com/triple-Mu/YOLOv8-TensorRT)

</details>

### 🌟 Other YOLO Variants
- 🏢 [**YOLOv6 Official**](https://github.com/meituan/YOLOv6) - Meituan's production YOLO
- 🏭 [**DAMO-YOLO**](https://github.com/tinyvision/DAMO-YOLO) - Industrial applications
- 🌐 [**YOLO-Universal**](https://github.com/jizhishutong/YOLOU)

---

## 🎯 Object Detection

### 🌟 NanoDet (Lightweight Detection)
> ⚡ **Super fast** and **ultra-lightweight** anchor-free detection

- 🚀 [**NanoDet**](https://github.com/RangiLyu/nanodet) - ![Stars](https://img.shields.io/github/stars/RangiLyu/nanodet?style=social)
- 📱 [**Android Demo**](https://github.com/nihui/ncnn-android-nanodet) - Official NCNN implementation

### 👤 BlazeFace
- 🔥 [**BlazeFace Android**](https://github.com/FeiGeChuanShu/ncnn_Android_blazeface) - Google's face detection

### 🎁 General Detection
- 🎁 [**Lite.AI.ToolKit**](https://github.com/DefTruth/lite.ai.toolkit) - Comprehensive AI toolkit
- 📱 [**YOLO Mobile**](https://github.com/wkt/YoloMobile) - Mobile-optimized YOLO

---

## 🖼️ Image Classification

<table>
<tr>
<td>

#### 🧠 Classic Models
- 🔬 [**SqueezeNet**](https://github.com/nihui/ncnn-android-squeezenet)
- 📱 [**MobileNet**](https://github.com/Revo-Future/ncnn_mobileNet)
- 🚀 [**MobileNetV2**](https://github.com/linguanghan/ncnn_mobilenetv2_andriod)

</td>
<td>

#### 📊 Features
- ✅ Lightweight architectures
- ⚡ Fast inference
- 📱 Mobile-optimized

</td>
</tr>
</table>

---

## ✂️ Image Segmentation

### 🎭 Portrait & Semantic Segmentation

| Model | Use Case | Platform | Stars |
|-------|----------|----------|-------|
| 👤 [**Portrait Seg**](https://github.com/leeys888/ncnn-portrait-segmentation) | Portrait | Mobile | ![](https://img.shields.io/github/stars/leeys888/ncnn-portrait-segmentation?style=social) |
| 🎬 [**Video Matting**](https://github.com/FeiGeChuanShu/ncnn_Android_RobustVideoMatting) | Real-time | Android | ![](https://img.shields.io/github/stars/FeiGeChuanShu/ncnn_Android_RobustVideoMatting?style=social) |
| 🎯 [**SAM Android**](https://github.com/slz929/SAM-Android-ncnn) | Segment Anything | Android | 🆕 2024 |
| 🏥 [**U-Net**](https://github.com/xuduo35/unet_mxnet2ncnn) | Medical/General | All | Classic |
| 🎨 [**BiSeNet**](https://github.com/CoinCheung/BiSeNet) | Semantic | All | SOTA |

---

## ✨ Super Resolution & Image Enhancement

<div align="center">

### 🌟 Real-ESRGAN & Waifu2x Family

![Super Resolution](https://img.shields.io/badge/Quality-4K%2B-purple?style=for-the-badge)
![Speed](https://img.shields.io/badge/Speed-Real--time-green?style=for-the-badge)

</div>

#### 🎨 Production-Ready Solutions

<table>
<tr>
<td width="50%">

##### 🖼️ Real-ESRGAN Series
- 🎯 [**Real-ESRGAN Vulkan**](https://github.com/xinntao/Real-ESRGAN-ncnn-vulkan)
  - GPU accelerated
  - 4K+ upscaling
  - Production-ready

- 📱 [**Android All-in-One**](https://github.com/tumuyan/RealSR-NCNN-Android)
  - RealSR, SRMD, RealCUGAN
  - Real-ESRGAN, Waifu2x
  - Anime4k support

</td>
<td width="50%">

##### 🎌 Waifu2x & Anime
- 💎 [**Waifu2x GUI**](https://github.com/AaronFeng753/Waifu2x-Extension-GUI)
  - Video/Image upscaling
  - Frame interpolation
  - Batch processing

- 🎨 [**Original Waifu2x**](https://github.com/nagadomi/waifu2x)
  - Anime-style art
  - Noise reduction
  - Classic model

</td>
</tr>
</table>

#### 🔬 Advanced Models
- 🌟 [**SRMD Vulkan**](https://github.com/nihui/srmd-ncnn-vulkan) - Super-resolution
- 📸 [**RealSR**](https://github.com/jixiaozhong/RealSR) - Real-world SR
- 🎨 [**Picture Enhancement**](https://github.com/JuZiSYJ/ncnn-picture-enhancement) - General enhancement
- 👤 [**GFPGAN**](https://github.com/FeiGeChuanShu/GFPGAN-ncnn) - Face restoration

---

## 👤 Face & Biometrics

### 🔍 Face Detection & Recognition

<div align="center">

| Model | Size | Speed | Accuracy |
|:-----:|:----:|:-----:|:--------:|
| 🏆 Ultra-Light | **1MB** | ⚡⚡⚡ | ⭐⭐⭐⭐ |
| 🎯 PFLD | **2MB** | ⚡⚡ | ⭐⭐⭐⭐⭐ |

</div>

- 🥇 [**Ultra-Light-Fast 1MB**](https://github.com/Linzaer/Ultra-Light-Fast-Generic-Face-Detector-1MB)
  - ![Size](https://img.shields.io/badge/Size-1MB-green)
  - Ultra lightweight
  - Real-time detection

- 📍 [**PFLD Landmark**](https://github.com/polarisZhao/PFLD-pytorch)
  - 98 facial landmarks
  - High accuracy
  - Mobile-optimized

### 👁️ Other Biometrics
- 🎯 [**Iris Landmarks**](https://github.com/digital-nomad-cheng/Iris_Landmarks_PyTorch) - Eye tracking

---

## 🏃 Pose Estimation & Human Tracking

<div align="center">

### 🎯 State-of-the-Art Models

![Pose](https://img.shields.io/badge/Real--time-Tracking-blue?style=for-the-badge)
![Mobile](https://img.shields.io/badge/Mobile-Optimized-green?style=for-the-badge)

</div>

#### 🌟 Featured Models

<table>
<tr>
<td width="33%">

**🔥 RTMPose**
- Snapdragon 865
- NCNN optimized
- High accuracy

</td>
<td width="33%">

**🎯 YOLOv8-Pose**
- Real-time
- Multi-person
- Edge devices

</td>
<td width="33%">

**⚡ BlazePose**
- MediaPipe based
- 33 keypoints
- Mobile-first

</td>
</tr>
</table>

#### 📚 Additional Models
- 🏃 **MoveNet** - Efficient single/multi-pose
- 🎭 **PoseNet** - Classic pose estimation
- 📱 **MobileNet Pose** - Lightweight variant

#### 📊 Resources
- 📈 [**Best Models 2024**](https://www.posetracker.com/news/best-human-pose-estimation-models-for-mobile-app-in-2024) - Comprehensive comparison

---

## 🎙️ Speech Recognition & ASR

<div align="center">

### 🎤 Sherpa-NCNN - Next-Gen Speech Recognition

![ASR](https://img.shields.io/badge/ASR-Real--time-red?style=for-the-badge)
![Offline](https://img.shields.io/badge/Mode-Offline-green?style=for-the-badge)
![Languages](https://img.shields.io/badge/Languages-5+-blue?style=for-the-badge)

</div>

#### 🚀 Main Repository
- 🎯 [**k2-fsa/sherpa-ncnn**](https://github.com/k2-fsa/sherpa-ncnn) ![Stars](https://img.shields.io/github/stars/k2-fsa/sherpa-ncnn?style=social)

#### ✨ Key Features

<table>
<tr>
<td width="50%">

**📱 Platform Support**
- ✅ iOS
- ✅ Android
- ✅ Linux
- ✅ macOS
- ✅ Windows
- ✅ Raspberry Pi
- ✅ VisionFive2
- ✅ LicheePi4A

</td>
<td width="50%">

**🌍 Language Support**
- 🇨🇳 Chinese
- 🇺🇸 English
- 🇯🇵 Japanese
- 🇰🇷 Korean
- 🇭🇰 Cantonese

</td>
</tr>
</table>

#### 🎯 Features
- ⚡ Real-time recognition
- 🔇 Voice Activity Detection (VAD)
- 🚫 No internet required
- 🎓 Next-gen Kaldi based

#### 📖 Documentation
- 📚 [**Complete Docs**](https://k2-fsa.github.io/sherpa/ncnn/) - Pre-trained models & guides

---

## 📝 OCR & Text Recognition

<div align="center">

### 📖 Lightweight OCR Solutions

![OCR](https://img.shields.io/badge/OCR-Multi--Language-orange?style=for-the-badge)
![Platform](https://img.shields.io/badge/Platform-Cross--Platform-blue?style=for-the-badge)

</div>

#### 🏆 Featured Projects

| Project | Languages | Platform | Highlights |
|---------|-----------|----------|------------|
| 🇨🇳 **chineseocr_lite** | Chinese | All | Super lightweight, vertical text |
| 🔧 [**ncnn_ocr**](https://github.com/sunsean21/ncnn_ocr-master) | Multi | Mobile | Profiling tool |
| 🌐 **WebAssembly OCR** | Multi | Browser | Browser-based |

#### 📊 Resources
- 📈 [**Top 10 OCR Models 2025**](https://www.kdnuggets.com/10-awesome-ocr-models-for-2025)

---

## 🎬 Video Processing

### 🎥 Frame Interpolation & Optical Flow

<div align="center">

![Video](https://img.shields.io/badge/Video-Processing-purple?style=for-the-badge)
![FPS](https://img.shields.io/badge/Output-60%2B%20FPS-green?style=for-the-badge)

</div>

#### 🌟 RIFE - Real-Time Frame Interpolation

- ⚡ **RIFE-ncnn-vulkan** - Production-ready Vulkan implementation
- 📊 [**ECCV 2022 RIFE**](https://github.com/hzwer/ECCV2022-RIFE)
  - ![Stars](https://img.shields.io/github/stars/hzwer/ECCV2022-RIFE?style=social)
  - Real-time interpolation
  - 60+ FPS output
  - Anime & video support

#### ✨ Features
- 🎯 Optical flow estimation
- 🎬 Video frame interpolation
- ⚡ GPU accelerated
- 📱 Mobile deployment

---

## 🎨 Stable Diffusion & Generative Models

<div align="center">

### 🖼️ AI Art Generation on Mobile

![Diffusion](https://img.shields.io/badge/Stable%20Diffusion-NCNN-blueviolet?style=for-the-badge)
![Mode](https://img.shields.io/badge/Mode-Offline-green?style=for-the-badge)

</div>

#### 🎯 Production Implementations

<table>
<tr>
<td>

**🎨 Stable Diffusion NCNN**
- [**EdVince/SD-NCNN**](https://github.com/EdVince/Stable-Diffusion-NCNN)
- ✅ txt2img support
- ✅ img2img support
- ✅ C++ implementation
- 📱 Mobile-ready

</td>
<td>

**🖌️ Features**
- 🎯 512x512 generation
- ⚡ Optimized inference
- 💾 Low memory usage
- 🔥 Vulkan acceleration

</td>
</tr>
</table>

#### 📚 Reference
- 🌐 [**SD WebUI**](https://github.com/AUTOMATIC1111/stable-diffusion-webui) - Desktop reference

---

## 📦 Model Collections & Zoo

<div align="center">

### 🗃️ Pre-trained Model Libraries

![Models](https://img.shields.io/badge/Models-1000%2B-brightgreen?style=for-the-badge)

</div>

#### 🏆 Curated Collections

| Repository | Models | Guides | Stars |
|------------|--------|--------|-------|
| 🎁 [**ncnn-models**](https://github.com/Baiyuetribe/ncnn-models) | 100+ | ✅ | ![](https://img.shields.io/github/stars/Baiyuetribe/ncnn-models?style=social) |
| 📦 [**ncnn_models**](https://github.com/nilseuropa/ncnn_models) | 50+ | ✅ | ![](https://img.shields.io/github/stars/nilseuropa/ncnn_models?style=social) |
| 🏛️ [**ncnn-assets**](https://github.com/nihui/ncnn-assets/tree/master/models) | Official | ✅ | Official |
| ⭐ [**awesome-ncnn**](https://github.com/zchrissirhcz/awesome-ncnn) | Curated | ✅ | ![](https://img.shields.io/github/stars/zchrissirhcz/awesome-ncnn?style=social) |

#### 🌐 Platform Support
- 💻 Windows, Mac, Linux
- 📱 Android, iOS
- 🌐 WebAssembly
- 📲 Uni-app

---

## 📱 Platform-Specific Deployment

### 🤖 Android

<details>
<summary>📱 <b>Click to expand Android resources</b></summary>

#### 📚 Comprehensive Guides
- 📖 [**PyTorch → NCNN Android**](https://medium.com/@freshtechyy/deployment-of-pytorch-model-using-ncnn-for-mobile-devices-part-2-ff28e9aaf0d6)
  - Complete deployment tutorial
  - NDK setup guide
  - CMake configuration

- 📦 [**ncnn-mobile**](https://github.com/dangbo/ncnn-mobile)
  - Android + iOS support
  - Sample projects
  - Best practices

#### 🔧 Requirements
- ✅ Android NDK
- ✅ CMake 3.10+
- ✅ OpenCV Android (optional)

</details>

### 🍎 iOS

<details>
<summary>📱 <b>Click to expand iOS resources</b></summary>

#### 📚 Swift Integration
- 🔗 [**C++ in Swift Guide**](https://anuragajwani.medium.com/how-to-consume-c-code-in-swift-b4d64a04e989)
  - Bridging header setup
  - Framework integration
  - Best practices

- 🎁 [**ncnn-swift**](https://github.com/zhuzilin/ncnn-swift)
  - Swift wrapper
  - CocoaPods support
  - Sample apps

#### ⚙️ Configuration
- ✅ Xcode 12+
- ✅ iOS 11+
- ✅ Vulkan/Metal support

</details>

### 🌐 WebAssembly

<details>
<summary>🌐 <b>Click to expand WASM resources</b></summary>

- 🎯 [**Portrait Segmentation WASM**](https://github.com/nihui/ncnn-webassembly-portrait-segmentation)
  - Browser-based inference
  - No server required
  - Real-time processing

#### 🚀 Features
- ✅ Zero installation
- ✅ Privacy-preserving
- ✅ Cross-browser support

</details>

### 🔧 OpenCV Integration

- 📷 [**opencv-mobile**](https://github.com/nihui/opencv-mobile)
  - Minimal OpenCV build
  - NCNN optimized
  - Reduced size

---

## ⚙️ Optimization & Quantization

<div align="center">

### 🚀 Performance Optimization

![INT8](https://img.shields.io/badge/Quantization-INT8-orange?style=for-the-badge)
![Speedup](https://img.shields.io/badge/Speedup-4x-brightgreen?style=for-the-badge)

</div>

### 🎯 INT8 Quantization

#### 📚 Official Resources
- 📖 [**INT8 Inference Guide**](https://github.com/Tencent/ncnn/blob/master/docs/how-to-use-and-FAQ/quantized-int8-inference.md)
  - Post-training quantization
  - Calibration workflow
  - KL & ACIQ methods

#### 🛠️ Tools & Libraries

<table>
<tr>
<td width="50%">

**🔧 Conversion Tools**
- ⚡ [**caffe-int8-tools**](https://github.com/BUG1989/caffe-int8-convert-tools)
- 🎓 [**ncnnqat**](https://github.com/ChenShisen/ncnnqat) - QAT support

</td>
<td width="50%">

**📊 Benefits**
- ⚡ 4× faster inference
- 💾 4× smaller models
- 📱 Lower memory usage
- 🔋 Better battery life

</td>
</tr>
</table>

### 🌋 Vulkan Optimization

#### 📚 Resources
- 🔥 [**NCNN Machine Vision 2025**](https://www.unitxlabs.com/resources/ncnn-machine-vision-system-2025/)
- ⚙️ [**Vulkan Optimization Notes**](https://github.com/Tencent/ncnn/wiki/vulkan-notes)

#### ✨ Key Features
- ✅ GPU acceleration
- ✅ Cross-platform
- ✅ Memory optimization
- ✅ Pipeline caching

---

## 🛠️ Tools & Utilities

### 🔄 Model Conversion & Editing

<table>
<tr>
<td width="50%">

#### 🔧 Conversion Tools
- ⚡ [**ONNX Simplifier**](https://github.com/daquexian/onnx-simplifier)
  - Model optimization
  - Graph simplification
  - Size reduction

- 🌐 [**ConvertModel.com**](https://convertmodel.com/)
  - Online converter
  - Multiple formats
  - Free to use

</td>
<td width="50%">

#### 🎨 Visual Tools
- 🖼️ [**ncnn-editor**](https://github.com/scarsty/ncnn-editor)
  - Visual model editing
  - Graph visualization
  - Debug support

- 📊 [**ncnn-small-board**](https://github.com/nihui/ncnn-small-board)
  - Model visualization
  - Layer inspection
  - Performance profiling

</td>
</tr>
</table>

### 🔗 Inference Helpers

- 🎁 [**InferenceHelper**](https://github.com/iwatake2222/InferenceHelper)
  - Unified interface
  - Multiple frameworks
  - Easy integration

- 📦 [**InferenceHelper_Sample**](https://github.com/iwatake2222/InferenceHelper_Sample)
  - Sample projects
  - Best practices
  - Quick start

### 🏗️ Build Tools

- 🔨 [**CMake Examples**](https://github.com/zchrissirhcz/cmake_examples)
  - CMake integration
  - Cross-platform builds
  - Modern CMake

---

## 📚 Tutorials & Documentation

### 📖 Official Documentation

<div align="center">

![Docs](https://img.shields.io/badge/Documentation-Comprehensive-blue?style=for-the-badge)
![Languages](https://img.shields.io/badge/Languages-EN%20%2B%20CN-green?style=for-the-badge)

</div>

<table>
<tr>
<td width="50%">

#### 🇺🇸 English Resources
- 📘 [**NCNN Wiki**](https://github.com/Tencent/ncnn/wiki)
- 🔨 [**How to Build**](https://github.com/Tencent/ncnn/wiki/how-to-build)
- 📖 [**PyTorch → NCNN Part 1**](https://medium.com/@freshtechyy/deployment-of-pytorch-model-using-ncnn-bceff5d846b0)
- 📱 [**PyTorch → NCNN Part 2**](https://medium.com/@freshtechyy/deployment-of-pytorch-model-using-ncnn-for-mobile-devices-part-2-ff28e9aaf0d6)
- 🆚 [**PyTorch vs ONNX vs NCNN**](https://medium.com/@nadirapovey/pytorch-vs-onnx-vs-ncnn-ee50115b6263)
- 🚀 [**Getting Started**](https://www.deepchecks.com/llm-tools/ncnn/)

</td>
<td width="50%">

#### 🇨🇳 Chinese Resources
- 🔬 [**ncnn_breakdown**](https://github.com/Zhengtq/ncnn_breakdown)
- 📝 [**Zhihu Column**](https://www.zhihu.com/column/c_1320446932913762304)
- 📚 [**CSDN Analysis**](https://blog.csdn.net/just_sort/article/details/111403398)
- 📖 [**Deep Dive Series**](https://blog.csdn.net/sinat_31425585/category_9312419.html)

</td>
</tr>
</table>

---

## 📊 Benchmarks & Performance

<div align="center">

### ⚡ Official Benchmark Results (Dec 2024)

![Performance](https://img.shields.io/badge/Performance-SOTA-brightgreen?style=for-the-badge)

</div>

#### 🏆 Key Metrics

<table align="center">
<tr>
<th>Metric</th>
<th>NCNN</th>
<th>Comparison</th>
<th>Improvement</th>
</tr>
<tr>
<td>⚡ Latency</td>
<td><b>Low</b></td>
<td>vs GPU</td>
<td><b>58.54% ↓</b></td>
</tr>
<tr>
<td>🚄 LLM Inference</td>
<td><b>Fast</b></td>
<td>vs Baseline</td>
<td><b>3.2× ↑</b></td>
</tr>
<tr>
<td>💪 Throughput</td>
<td><b>High</b></td>
<td>vs NPU</td>
<td><b>2× ↑</b></td>
</tr>
<tr>
<td>📱 Platforms</td>
<td colspan="3"><b>10+ Supported</b></td>
</tr>
</table>

#### 📊 Benchmark Resources

- 📈 [**Official Benchmarks**](https://github.com/Tencent/ncnn/blob/master/benchmark/README.md)
- 🔥 [**OpenBenchmarking**](https://openbenchmarking.org/test/pts/ncnn)

### 🆚 Framework Comparisons

<table>
<tr>
<td width="33%" align="center">

**🔬 embedded-ai.bench**
- [Benchmark Suite](https://github.com/AI-performance/embedded-ai.bench)
- NCNN vs TNN vs MNN
- TFLite comparison

</td>
<td width="33%" align="center">

**📱 Mobile AI Bench**
- [Xiaomi Benchmark](https://github.com/XiaoMi/mobile-ai-bench)
- Real device testing
- Production workloads

</td>
<td width="33%" align="center">

**⚡ vs TensorRT**
- [Detailed Comparison](https://medium.com/@reza_mahmoudi/the-difference-between-ncnn-and-tensorrt-9e869390fe00)
- Use case analysis
- Performance metrics

</td>
</tr>
</table>

---

## 📄 Research Papers & Articles (2024-2025)

<div align="center">

### 📚 Latest Academic Research

![Research](https://img.shields.io/badge/Research-2024--2025-red?style=for-the-badge)

</div>

### 🎯 YOLO Family Comparisons

<table>
<tr>
<td>

#### 📊 Performance Studies
- 📈 [**YOLOv8/v9/v10/v11 Comparison**](https://www.mdpi.com/2076-3417/15/6/3154)
  - Comprehensive evaluation
  - Multiple datasets
  - Production insights

- 🏗️ [**YOLOv11 Architecture**](https://arxiv.org/html/2410.17725v1)
  - Key enhancements
  - Design principles
  - Technical deep dive

</td>
<td>

#### 🔥 Application Papers
- 🚨 [**Smoke/Fire Detection**](https://www.mdpi.com/2571-6255/8/1/26)
  - YOLOv9/v10/v11
  - Real-world testing
  - Safety applications

- 🌱 **Weed Detection** - Agricultural AI
- 📄 **Document Analysis** - OCR pipelines
- 🖼️ **Stamp Detection** - Document processing

</td>
</tr>
</table>

### 🚀 Mobile AI Trends

- 🎯 [**NCNN Machine Vision 2025**](https://www.unitxlabs.com/resources/ncnn-machine-vision-system-2025/)
  - Latest optimization techniques
  - Industry trends
  - Best practices

- 🧠 [**Post-Transformer Architectures**](https://www.latent.space/p/2024-post-transformers)
  - State Space Models
  - RWKV architectures
  - Future directions

---

<div align="center">

## 🤝 Contributing

We welcome contributions! 🎉

**How to contribute:**
- ⭐ Star this repository
- 🍴 Fork and submit PRs
- 🐛 Report issues
- 💡 Suggest new resources

</div>

---

<div align="center">

## 📜 License

This collection is provided as-is for educational and reference purposes.

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg?style=for-the-badge)](https://opensource.org/licenses/MIT)

</div>

---

<div align="center">

## ⏰ Last Updated

**🗓️ January 2025** - Continuously updated with the latest NCNN projects and resources

<br>

### 🌟 Star History

[![Star History Chart](https://api.star-history.com/svg?repos=umitkacar/awesome-ncnn-collection&type=Date)](https://star-history.com/#umitkacar/awesome-ncnn-collection&Date)

<br>

---

**Made with ❤️ for the NCNN Community**

[⬆ Back to Top](#-awesome-ncnn-collection)

</div>
